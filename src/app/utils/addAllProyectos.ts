...
import { createParticipa, createProyectoItem, fetchInvestigadorByNombreAutor, fetchProyectoByCode, ProyectoType } from "../../../db/db";
...
const validateProyectoToAdd = async (proyecto: ProyectoType) => {
    const {codigo, ip, inicio, fin} = proyecto;
        ...
        const isStartDateBeforeEndDate = (inicio && fin && inicio.valueOf() < fin.valueOf());
...
export const addAllProyectos = async (proyectos: ProyectoType[]) => {
    ...
    for (const proyecto of proyectos) {
        ...
        if (!errorMessage) {
            ...
            const investigadorId = (await fetchInvestigadorByNombreAutor(proyecto.ip))![0].id;
